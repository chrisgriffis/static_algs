...
int demo(std::ostream& sout) {
    ...
    auto DB_SCHEMA_EXAMPLE =
        db_row_schema<
        std::string,    // path;
        winrt::uint64_t,// folderKey;
        winrt::uint64_t,// thumbId;
        winrt::uint32_t,// fileAttributes;
        winrt::uint32_t,// arbitraryKey;
    ...
    sout << "DB_SCHEMA_EXAMPLE col headings: " << endl << DB_SCHEMA_EXAMPLE.headings_as_list() << endl << endl;
    sout << "DB_SCHEMA_EXAMPLE to_single_record, empty: " << endl << DB_SCHEMA_EXAMPLE.to_single_record() << endl << endl;
    auto rec = DB_SCHEMA_EXAMPLE.to_single_record("a/cool/path", 7, 3, 67, 23, 4, 6, 45, 7);
    sout << "DB_SCHEMA_EXAMPLE to_single_record, with values: " << endl << rec << endl << endl;
    sout << "DB_SCHEMA_EXAMPLE to_single_record, converted values: " << endl << rec.converted_row_values() << endl << endl;
